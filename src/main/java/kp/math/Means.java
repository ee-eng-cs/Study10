...
public class Means {
	...
	private static void computeMeansWithTeeing(Supplier<Stream<Integer>> streamSupplier) {
		...
		System.out.printf("Supplied sequence%s%n", streamSupplier.get().collect(Collectors.toList()));
		/*
		 * Harmonic Mean
		 */
		...
		final Collector<Integer, ?, Double> geometricMeanCollector = Collectors.teeing(/*-*/
				...
				Collectors.summingDouble(arg -> Math.log(arg)), /*-*/
				Collectors.counting(), /*-*/
		...
		System.out.printf(" →  Quadratic mean[%.2f]%n", quadraticMean);
